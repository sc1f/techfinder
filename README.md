...
- **Controls.**
  ...
  - The bottom row, left to right: Settings, Grid, Frame (sensor or film format), and a Lenses | Movements switch for the row above. Settings sets the ISO, aperture and shutter limits of your equipment, and opens the lens library (add, edit and choose lenses and their image circles).
  - The controls never cover the camera image, which sits at the top and never moves or resizes. With Movements on, the movement controls take the lens selector's place, so nothing else shifts. On a short screen such as the iPhone SE, the image is a little smaller to leave the controls room.
  - Turning movements on keeps the camera's zoom, so nothing changes at zero. The camera widens to fit the whole image circle only when the moved frame nears the edge of what it sees, or the overview opens, and stays wide until movements turn off. That switch, and choosing another lens or format, jumps straight to the new zoom (often another of the iPhone's cameras) under a brief blur, as the Camera app does when it changes lenses. Pinching still zooms smoothly.
...
- **Image circle and movements.**
  ...
  - Switch to Movements, choose Rise or Shift, and move only that axis: with the arrows, by swiping the value, or by dragging the image. Steps are 0.5 mm. Tap the value for a menu to reset it, or both axes, to zero. Double-tapping the image also resets the current axis, with a few seconds to undo.
  - The result view pans to the moved frame as the photo would look, keeping the frame the size it has without movements, so nothing jumps when they turn on. The dashed-circle button shows the overview, which shows the whole image circle, the unmoved frame (dashed) and the moved frame.
  - With an ultra-wide lens, the image circle or a moved frame can reach past what the iPhone camera sees, even at its widest. That area is hatched, with the camera's edge dashed, and "Wider than the iPhone can see" shows at the bottom of the image. The overview zooms out to show the whole image circle.
...
- **Too wide.** When the setup is wider than the iPhone's ultra-wide can see, the frame turns orange and "Wider than the iPhone can see" shows at the bottom of the image.
