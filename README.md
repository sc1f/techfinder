...
- **Controls.**
  ...
  - Turning movements on zooms the camera out only as far as the frame can travel within the image circle and your camera's limits, so the camera never changes mid-movement; the frame keeps its size. The overview fits the whole image circle. Those switches, and choosing another lens or format, jumps straight to the new zoom (often another of the iPhone's cameras) under a brief blur, as the Camera app does when it changes lenses. Pinching still zooms smoothly.
...
- **Image circle and movements.**
  ...
  - Switch to Movements, choose Rise or Shift, and move only that axis: with the arrows, by swiping the value, or by dragging the image. Steps are 0.5 mm. Tap the value for a menu to reset it, or both axes, to zero. Double-tapping the image also resets the current axis.
