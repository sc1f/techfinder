...
///
/// Runs on its own queue and renders about 12 frames a second at a few dozen pixels wide.
final class BackdropRenderer: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    private let interval = CMTime(value: 1, timescale: 12)
    private let outputWidth: CGFloat = 60
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        let blurred = small.clampedToExtent().applyingGaussianBlur(sigma: 2.5).cropped(to: extent)
